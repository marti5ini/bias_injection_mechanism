...
from run_cd_algorithms import run_causal_discovery
...
if __name__ == '__main__':
    ...
    for i, (type_st, structure) in enumerate(combined.items()):
        ...
        # fig.suptitle(f'{type_st.upper()}, A->Y = {A_Y}')
        changes = dict()
        for j, name in enumerate(structure):
            ...
            for k, parameter2 in enumerate(parameters2_list):
                ...
                for _ in range(10):
                    df, ground_truth = generate_data_new(structure=name, structure_type=type_st,
                                       bias=parameter2, sensitive_attributes=['A'], A_Y=A_Y,
                                       parameter1=parameter1, n_sample=n_sample, path=path, spd=False)

                    causal_matrix = run_causal_discovery(structure=name, path=path, parameter2=parameter2,
                                                         cd_algorithm=cd_algo)
            ...
            #plot_using_graphviz(changes[name], parameters2_list, name, axs[j], ground_truth)
            plot_colored_graph(changes[name], parameters2_list, name, axs[j], ground_truth)
        #grid_subplots(changes, cd_algo, type_st, path)
